...
long double weights[NUM_FEATURES]= {0,0,0,0,0,0,0,0,0};

long double regularization_strength = 10000;
long double learning_rate = 0.000001;


long double ascent[NUM_FEATURES];
...
void calculate_cost_gradient(int ind, int Y_batch) {

	long double distance,sum=0;
//	vector<long double> dw;
	long double di[NUM_FEATURES];

	for(int i=0; i<NUM_FEATURES; i++) {
		sum+= x_train_array[ind][i]*weights[i];
	...
	if(distance<0) {
		...
		for(int i=0; i<NUM_FEATURES; i++) {
			di[i]=x_train_array[ind][i]- regularization_strength * Y_batch * x_train_array[ind][i];
		}
...
long double  compute_cost() {
	...
	long double distance[training_samples];
	long double sum=0;
	...
	for(int i =0; i<training_samples; i++) {
		...
		sum=0;
		for(int j=0; j<NUM_FEATURES; j++) {
			sum+= x_train_array[i][j]*weights[j];

		}
		distance[i] = 1- sum*y_train_label_array[i];
		if (distance[i]<0) {
			distance[i]=0;
		}
	}
...
int sgd() {
	...
	for(int epoch=1; epoch<=max_epochs; epoch++) {
		...
		for(int i=0; i<training_samples; i++) {
			calculate_cost_gradient(i, y_train_label_array[i]);
			for(int j=0; j<NUM_FEATURES; j++)
